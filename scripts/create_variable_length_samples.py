...
def create_prompt(passage_list, selected_indexes, query):
    ...
    prefix = (f'I will provide you with {num_selected_passages} passages, each indicated by a numerical identifier []. '
...
def sample_data(json_objects, num_passages, num_samples, include_original):
    ...
    for obj in json_objects:
        ...
        # print(response)
        if include_original:
            obj["conversations"][prompt_index]["value"] = obj["conversations"][prompt_index]["value"].replace(
                f'e.g., [4] > [2], ', f'e.g., [2] > [1], ')
            new_objs.append(obj)
        for k in range(num_samples):
            n = random.randint(2, len(passages) - 1) if include_original else random.randint(2, len(passages))
            ...
            if len(obj["conversations"]) == 3:
                convs.append({'from': 'system', 'value': 'You are RankLLM, an intelligent assistant that can rank passages based on their relevancy to the query.'})    
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_samples", type=int, default=1, help="number of samples generated for each input conversation"
    )
    parser.add_argument(
        "--num_passages",
        type=int,
