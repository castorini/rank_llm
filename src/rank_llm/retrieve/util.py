...
def get_cache_home():
    ...
    if custom_dir is not None and custom_dir != '':
        print('custom')
        return custom_dir
    return os.path.expanduser(os.path.join(f'~{os.path.sep}rank_llm', "retrieve_results"))


def download_and_unpack_index(url, index_directory='files', local_filename=False,
    ...
    if os.path.exists(file_path):
        if not force:
            ...
            return file_path
        if verbose:
            print(f'{file_path} already exists, but force=True, removing {file_path} and fetching fresh copy...')
        os.remove(file_path)
